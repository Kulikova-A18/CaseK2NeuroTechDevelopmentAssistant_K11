...
graph TB
    ...
    subgraph "Наша система"
        ...
        subgraph "Flask Server"
            ...
            LLM[LLM Report Module]
            Config[Config Manager]
        end
        KB[Kanban Board<br/>Web Interface]
        subgraph "Data Storage"
            CSV[CSV Files<br/>users.csv, tasks.csv<br/>docs.csv, events.csv]
            Cache[Redis Cache]
            Logs[Log Files]
        end
    end
    ...
    %% Внутренние взаимодействия сервера
    API -->|Authentication| Auth
    API -->|Task Operations| Tasks
    API -->|User Management| Users
    API -->|Export Requests| Export
    API -->|Report Generation| LLM
    
    Auth -->|Read/Write| CSV
    Tasks -->|Read/Write| CSV
    Users -->|Read/Write| CSV
    
    Auth -->|Cache Sessions| Cache
    Tasks -->|Cache Data| Cache
    Export -->|Cache Results| Cache
    
    LLM -->|API Call| OpenAI
    
    Config -->|Read| CSV
    Config -->|Write Logs| Logs
```
